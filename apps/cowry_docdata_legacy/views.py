...
from apps.cowry_docdata.models import DocDataPaymentOrder, DocDataPaymentLogEntry
from .adapters import DocdataLegacyPaymentAdapter

logger = logging.getLogger(__name__)

payment_adapter = DocdataLegacyPaymentAdapter()


# TODO: limit status change notifications to docdata IPs
class StatusChangedNotificationLegacyView(generics.GenericAPIView):
    def get(self, request, *args, **kwargs):
        if 'order' in request.QUERY_PARAMS:
            order = request.QUERY_PARAMS['order']
            if re.match('^[0-9]+$', order):
                ...
                status_log.save()
                payment_adapter.update_payment_status(payment, status_changed_notification=True)
