...
class Migration(DataMigration):
    ...
    def _get_fee_percentage(self, payout_rule):
        """
        ...
        if payout_rule == PayoutRules.five:
            # 5%
            return decimal.Decimal('0.05')

        elif payout_rule == PayoutRules.seven:
            # 7%
            return decimal.Decimal('0.07')

        elif payout_rule == PayoutRules.twelve:
            # 12%
    ...
    def forwards(self, orm):
        ...
        for payout in orm.Payout.objects.all():
            if not payout.organization_fee and not payout.amount_raised:
                ...
                payout.amount_raised = payout.amount_payable / inverse_factor

                # Calculate fee the usual way
                payout.organization_fee = payout.amount_raised * fee_factor

                # Assert they match
                assert payout.amount_payable + payout.organization_fee == payout.amount_raised
