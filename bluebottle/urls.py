...
# These are used to make the css, images and fonts folder accessible in the root folder when debug is turned on. This
# is needed for the sprites generated by compass / scss to work properly. NOTE: The webserver needs to be configured
# with aliases for these. If we want to serve these from a second server, we'll have to find a solution for this.
urlpatterns += static('css', document_root=os.path.join(settings.STATICFILES_DIRS[0], 'css'))
urlpatterns += static('images', document_root=os.path.join(settings.STATICFILES_DIRS[0], 'images'))
urlpatterns += static('fonts', document_root=os.path.join(settings.STATICFILES_DIRS[0], 'fonts'))
