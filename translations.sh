...
ROOT="$PWD"
APPS_ROOT="$PWD/apps"
BB_ROOT="$PWD/../bluebottle/bluebottle"

MAKEMESSAGES="$MANAGE_PY makemessages -l $SOURCE_LANGUAGE --no-wrap -e hbs,html,txt $SETTINGS"
MAKEJSMESSAGES="$MANAGE_PY makemessages -l $SOURCE_LANGUAGE --include=../bluebottle/bluebottle --ignore=djangojs.js  --ignore=node_modules --no-wrap -e js -i compressed -d djangojs $SETTINGS"
